...
```{r reading in modulize output mouse,warning=FALSE, message=FALSE}
...
```{r reading in modulize output zebrafish,warning=FALSE, message=FALSE}

# reading in modulize output
ASeventList <- loadTSVs("./data/modulize/Zebrafish")
...
# adding a colum to the data that indicates for each event wether it is regulated and if so if it is up or down regulated
modulize_regulated <- ASeventList[[3]] %>% select(event_id, junction_name, gfp_median_psi, usp39_median_psi, usp39.gfp_median_dpsi, denovo) %>%
  mutate(regulation = case_when((event_id %in% significant_events$event_id) & usp39.gfp_median_dpsi > 0 ~ "up",
                                  (event_id %in% significant_events$event_id) & usp39.gfp_median_dpsi < 0 ~ "down",
                                  ((!event_id %in% significant_events$event_id)) ~ "no")) %>% 
    mutate(regulation=factor(regulation, levels=c("no", "down", "up")))
...
```{r max ent scores Zebrafish, warning=FALSE}

# in case this code has already been run hte splice strength data frame is loaded otherwise it will be created and saved in the data folder
rds_path <- ("./data/rds_objects/Zebrafish_SSDF.rds")

if (!file.exists(rds_path)) {
  # read in regulated a5ss event granges objects
  Zebrafish_regulated <- readRDS("./data/rds_objects/Zebrafish_A5SSranges.rds")
 
  # read in not regulated a5ss event granges objects
  Zebrafish_not_regulated <- readRDS("./data/rds_objects/Zebrafish_nonRegA5SSranges.rds")
  
  A5SSranges <- Zebrafish_regulated
  nonRegulatedA5SSranges <- Zebrafish_not_regulated
  
  
canonicalSS <- endoapply(A5SSranges, function(gr){
    gr$exon_length <- width(gr)
    gr %>% plyranges::slice(which.min(usp39.gfp_median_dpsi))
}) %>% unlist %>% resize(., 1, fix="end") %>% `seqlevelsStyle<-`("UCSC")

alternativeSS <- endoapply(A5SSranges, function(gr){
    gr$exon_length <- width(gr)
    gr %>% plyranges::slice(which.max(usp39.gfp_median_dpsi))
}) %>% unlist %>% resize(., 1, fix="end") %>% `seqlevelsStyle<-`("UCSC")

# Non-regulated
nonRegCanonicalSS <- endoapply(nonRegulatedA5SSranges, function(gr){
    gr$exon_length <- width(gr)
    gr %>% plyranges::slice(which.max(gfp_median_psi))
}) %>% unlist %>% resize(., 1, fix="end") %>% `seqlevelsStyle<-`("UCSC")

nonRegAlternativeSS <- endoapply(nonRegulatedA5SSranges, function(gr){
    gr$exon_length <- width(gr)
    gr %>% plyranges::slice(which.min(gfp_median_psi))
}) %>% unlist %>% resize(., 1, fix="end") %>% `seqlevelsStyle<-`("UCSC")
    
    
  calc_splicesite_strength <- function(gr, set, A5SS){
    #gr <- nonRegAlternativeSS
    strengths <- gr %>%
        flank(., 6, start = FALSE) %>% promoters(., upstream = 3, downstream = 6) %>%
        getSeq(Drerio, .) %>%
        as.character() 
    
    filtered_vector <- strengths[!grepl("N", strengths)]
    filtered_out_vector <- strengths[grepl("N", strengths)]
    strengths <- VarCon::calculateMaxEntScanScore(filtered_vector, 5)%>%
        as.numeric()   
    
    cat("removing events: ", names(filtered_out_vector), "\n with sequences", filtered_out_vector,"\n")
    
    
    gr <- gr[!(gr$event_id %in% names(filtered_out_vector)), ]
    return(
        data.frame(
            set,
            A5SS,
  ...
  # filtering the events   
  eventsToKeep <- canonicalSS$event_id[canonicalSS$gfp_median_psi > 0.5]
canonicalSS <- canonicalSS %>% plyranges::filter(event_id %in% eventsToKeep)
alternativeSS <- alternativeSS %>% plyranges::filter(event_id %in% eventsToKeep)

eventsToKeep <- nonRegCanonicalSS$event_id[nonRegCanonicalSS$gfp_median_psi > 0.5]
nonRegCanonicalSS <- nonRegCanonicalSS %>% plyranges::filter(event_id %in% eventsToKeep)
nonRegAlternativeSS <- nonRegAlternativeSS %>% plyranges::filter(event_id %in% eventsToKeep)

# remove faulty seqlevel
seqlevels(canonicalSS, pruning.mode="coarse") <- setdiff(seqlevels(nonRegAlternativeSS), "KN150449.1")
seqlevels(alternativeSS, pruning.mode="coarse") <- setdiff(seqlevels(nonRegAlternativeSS), "KN150449.1")
seqlevels(nonRegCanonicalSS, pruning.mode="coarse") <- setdiff(seqlevels(nonRegAlternativeSS), "KN150449.1")
seqlevels(nonRegAlternativeSS, pruning.mode="coarse") <- setdiff(seqlevels(nonRegAlternativeSS), "KN150449.1")
